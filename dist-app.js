...
const fmt = (n,digits=2) => Number.isFinite(n) ? Number(n.toFixed(digits)).toLocaleString('en-US',{maximumFractionDigits:digits}) : '—';
const id = () => globalThis.crypto?.randomUUID?.() ?? `e${Date.now().toString(36)}${Math.random().toString(36).slice(2)}`;
...
function sampleArc(center,radius,start,sweep,tolerance=.5) {
...
function polylinePoints(e,tolerance=.5) {
...
function emptyBounds(){return {minX:Infinity,minY:Infinity,maxX:-Infinity,maxY:-Infinity};}
function includePoint(b,p){if(finitePoint(p)){b.minX=Math.min(b.minX,p.x);b.minY=Math.min(b.minY,p.y);b.maxX=Math.max(b.maxX,p.x);b.maxY=Math.max(b.maxY,p.y);}return b;}
function unionBounds(a,b){a.minX=Math.min(a.minX,b.minX);a.minY=Math.min(a.minY,b.minY);a.maxX=Math.max(a.maxX,b.maxX);a.maxY=Math.max(a.maxY,b.maxY);return a;}
const validBounds=b=>b&&Number.isFinite(b.minX)&&Number.isFinite(b.minY)&&Number.isFinite(b.maxX)&&Number.isFinite(b.maxY);
const overlaps=(a,b)=>a.minX<=b.maxX&&a.maxX>=b.minX&&a.minY<=b.maxY&&a.maxY>=b.minY;
const contains=(a,b)=>a.minX<=b.minX&&a.minY<=b.minY&&a.maxX>=b.maxX&&a.maxY>=b.maxY;
...
const transformPoint=(p,m)=>point(p.x*m[0]+p.y*m[2]+m[4],p.x*m[1]+p.y*m[3]+m[5]);
...
const rotation=(r,p=point())=>multiplyMatrix(translation(p.x,p.y),multiplyMatrix([Math.cos(r),Math.sin(r),-Math.sin(r),Math.cos(r),0,0],translation(-p.x,-p.y)));
const scaling=(s,p=point())=>[s,0,0,s,p.x*(1-s),p.y*(1-s)];
function reflection(a,b){const t=angle(a,b),c=Math.cos(2*t),s=Math.sin(2*t);return multiplyMatrix(translation(a.x,a.y),multiplyMatrix([c,s,s,-c,0,0],translation(-a.x,-a.y)));}
...
function entityLength(e){if(e.type==='CIRCLE')return TAU*e.radius;if(e.type==='ARC')return (normalizeAngle(e.end-e.start)||TAU)*e.radius;return geometry(e,.1).paths.reduce((total,path)=>total+path.slice(1).reduce((s,p,i)=>s+distance(path[i],p),0),0);}
function entityArea(e){if(e.type==='CIRCLE')return Math.PI*e.radius**2;if(e.type==='ELLIPSE')return Math.PI*dot(e.major,e.major)*e.ratio;if((e.type==='LWPOLYLINE'||e.type==='POLYLINE')&&e.closed){const p=e.points,o=p[0]??point();let area=0;for(let i=0;i<p.length;i++){const a=p[i],b=p[(i+1)%p.length];area+=cross(sub(a,o),sub(b,o))/2;const arc=bulgeArc(a,b,a.bulge??0);if(arc)area+=arc.radius*arc.radius*(arc.sweep-Math.sin(arc.sweep))/2;}return Math.abs(area);}return null;}
class SpatialIndex {
...
return {TAU, EPS, point, add, sub, mul, dot, cross, length, distance, midpoint, angle, normalizeAngle, clone, finitePoint, fmt, id, closestOnSegment, lineIntersection, circumcircle, arcThrough, bulgeArc, sampleArc, polylinePoints, emptyBounds, includePoint, unionBounds, validBounds, overlaps, contains, geometry, snapPoints, identity, transformPoint, multiplyMatrix, translation, rotation, scaling, reflection, transformEntity, entityLength, entityArea, SpatialIndex};
})();
__modules["dxf.js"] = (()=>{
const {point, id, TAU, geometry, transformEntity, multiplyMatrix, translation, normalizeAngle, distance, angle, finitePoint} = __modules["geometry.js"];
...
function aciColor(index){return ACI[Math.max(0,Math.min(255,Math.abs(Math.trunc(index??7))))]??'#ffffff';}
...
return {aciColor, parseDXF, writeDXF};
})();
__modules["model.js"] = (()=>{
const {clone,id} = __modules["geometry.js"];
...
const {geometry,emptyBounds,unionBounds,validBounds,point,SpatialIndex,overlaps} = __modules["geometry.js"];
const SHADER=`
...
__modules["demo.js"] = (()=>{
const {id,point,TAU,transformEntity,rotation,translation} = __modules["geometry.js"];
function createDemo(){
  const layers=[
...
function icon(name,cls=''){return `<svg class="icon ${cls}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.55" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="${paths[name]??paths.block}"/></svg>`;}
function applyIcons(root=document){for(const el of root.querySelectorAll('[data-icon]'))el.innerHTML=icon(el.dataset.icon);}

return {icon, applyIcons};
})();
__modules["app.js"] = (()=>{
const {point,add,sub,mul,dot,cross,length,distance,midpoint,angle,fmt,id,clone,TAU,EPS,geometry,snapPoints,emptyBounds,includePoint,unionBounds,validBounds,overlaps,contains,closestOnSegment,lineIntersection,arcThrough,bulgeArc,normalizeAngle,transformEntity,translation,rotation,scaling,reflection,entityLength,entityArea} = __modules["geometry.js"];
const {DocumentModel} = __modules["model.js"];
const {CADRenderer} = __modules["renderer.js"];
const {parseDXF,writeDXF} = __modules["dxf.js"];
const {createDemo} = __modules["demo.js"];
const {icon,applyIcons} = __modules["icons.js"];
